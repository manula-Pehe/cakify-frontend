...
import { inquiryService, Inquiry, InquiryStatus } from "@/services/inquiryService";
...
const AdminInquiries = () => {
  ...
  const handleReply = (inquiry: Inquiry) => {
    ...
    setReplyText(inquiry.adminNotes || "");
  ...
  const sendReply = async () => {
    ...
    if (selectedInquiry) {
      try {
        // Update status to IN_PROGRESS and add admin notes
        await inquiryService.updateStatus(selectedInquiry.id, {
  ...
  const markAsResolved = async (inquiryId: string) => {
    try {
      await inquiryService.updateStatus(inquiryId, {
